...
#./searchConvert2pic.sh ../dessin_test/ ../dessin_pic/
...
then
	echo
	echo -e $BPurple " Graphic convert .shr or .pnt to .pic" $Color_Off
	echo
else
echo -e $BPurple
	iix gothic " Graphic convert .shr to .pic"
echo -e $Color_Off
fi
...
then
	echo
	echo -e $BRed "$1 does not exist" $Color_Off	
	echo
	exit $ERROR_SH_folder
fi
...
for file in `find . -type f -iname "$checkingExt2"`	# recursive but missing base founded
do
	filepath=$(echo "$file")
	filepath="${filepath#?}" 	# removes first character
	filepath="${filepath#?}" 	# removes first character
...
do
	...
	echo -e $Green "Doing : " "$convm" "-rled" "+lower" "$checkoriginfile" $Color_Off
	# "$convm" "-rled" "+lower" $checkoriginfile 2> /dev/null
	"$convm" "-rled" "+lower" "$checkoriginfile"
	aError=$?
	if [ $aError -ne 0 ]
	then
		echo
